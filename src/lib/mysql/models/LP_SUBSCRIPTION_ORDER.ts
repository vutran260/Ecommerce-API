...
export type LP_SUBSCRIPTION_ORDERId =
  LP_SUBSCRIPTION_ORDER[LP_SUBSCRIPTION_ORDERPk];
export type LP_SUBSCRIPTION_ORDEROptionalAttributes = 'createdAt' | 'updatedAt';
export type LP_SUBSCRIPTION_ORDERCreationAttributes = Optional<
  LP_SUBSCRIPTION_ORDERAttributes,
  LP_SUBSCRIPTION_ORDEROptionalAttributes
>;

export class LP_SUBSCRIPTION_ORDER
  extends Model<
    LP_SUBSCRIPTION_ORDERAttributes,
    LP_SUBSCRIPTION_ORDERCreationAttributes
  >
  implements LP_SUBSCRIPTION_ORDERAttributes
{
  subscriptionId!: string;
  orderId!: number;
  createdAt!: Date;
        ...
        indexes: [
          {
            ...
            unique: true,
            using: 'BTREE',
            fields: [{ name: 'subscription_id' }, { name: 'order_id' }],
          },
          {
            name: 'order_id',
            using: 'BTREE',
