...
export class StoreEndpoint {
  ...
  private getStores = async (req: PaginationRequest, res: Response) => {
    const results = await this.storeUsecase.GetStoreList(
      req.filterList,
      req.order,
      req.paging,
    );
  ...
  private rejectCreatingStoreRequest = async (
    req: ProtectedRequest,
    res: Response,
  ) => {
    const Id = req.params.id;
    const remark = req.body.remark;
    const results = await this.storeUsecase.RejectCreateStoreRequest(
      Id,
      remark,
    );
    return ResponseData(results, res);
  ...
  private approveCreatingStoreRequest = async (
    req: ProtectedRequest,
    res: Response,
  ) => {
    const Id = req.params.id;
    const remark = req.body.remark;
    const results = await this.storeUsecase.ApproveCreateStoreRequest(
      Id,
      remark,
    );
    return ResponseData(results, res);
